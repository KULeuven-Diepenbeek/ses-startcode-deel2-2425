...
public class CandyCrushBoardUI extends Region {
    private final CandyCrushGame model;
    private final int candySize;

    public CandyCrushBoardUI(CandyCrushGame model) {
        this.model = model;
        candySize = 40;
        update();
    }
    ...
    public void update(){
        ...
        for (var pos : model.getPositions()) {
            int candy = model.getCandyAt(pos[0], pos[1]);
            Node shape = makeCandyShape(pos[0], pos[1], candy);
            getChildren().add(shape);
            // TODO: uncomment this and the methods it calls once you have implemented getPotentialSwitchesOf
            /*
            for (var sw : model.getPotentialSwitchesOf(pos)) {
    ...
    */
    ...
    private Node makeCandyShape(int row, int col, int candy) {
        var selected = model.isSelected(row, col);
        return switch(candy) {
    ...
    public int[] getPositionOfClicked(MouseEvent me){
        ...
        if (model.isValidPosition(row, column)) {
